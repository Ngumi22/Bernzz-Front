...
const ProductCard: React.FC<CardProps> = ({ product, showActions }) => {
  ...
  return (
    <Card className="group relative group/item w-[15rem] p-4">
      <div className="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8 h-3/4">
        <Image
          src={
            product.images.main
      ...
      {showActions && (
        <div className="absolute top-4 right-3 grid grid-flow-row gap-3 group/edit invisible group-hover/item:visible p-1">
          ...
          <button
            ...
            <p className="absolute invisible group-hover/icon:visible right-9 top-2 bottom-0 text-xs 2xl:text-md text-center w-36 my-auto text-white font-semibold">
              Order on WhatsApp
            </p>
          </button>
        </div>
